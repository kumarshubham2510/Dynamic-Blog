...
def register():
    ...
    if(form.validate_on_submit()):
        ...
        old_user=db.session.execute(db.select(Users).where(Users.email==email)).scalar()
        if(old_user):
            flash("Users already exists .User Login")
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to regiser and login to comment")
            return redirect(url_for(login))
        else:
